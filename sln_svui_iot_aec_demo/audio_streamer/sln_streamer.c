...
/* Backup structure used for resuming playback from a previous pause */
static streamer_local_file_t local_active_file_backup;
...
/*!
 * @brief Backup the active playback progress in order to resume it later
 *
 */
static void SLN_STREAMER_BackupPlayback(void)
{
    local_active_file_backup.filename = local_active_file_desc.filename;
    local_active_file_backup.len = local_active_file_desc.len;
    local_active_file_backup.offset = local_active_file_desc.offset;

    local_active_file_desc.filename = NULL;
    local_active_file_desc.len = 0;
    local_active_file_desc.offset = 0;
}

/*!
 * @brief Restore a previously backed up playback progress
 *
 */
static void SLN_STREAMER_RestorePlayback(void)
{
    local_active_file_desc.filename = local_active_file_backup.filename;
    local_active_file_desc.len = local_active_file_backup.len;
    local_active_file_desc.offset = local_active_file_backup.offset;
}
...
bool SLN_STREAMER_IsPaused(streamer_handle_t *handle)
{
    return handle->audioPaused;
}

bool SLN_STREAMER_IsEOS(streamer_handle_t *handle)
{
    return handle->eos;
}
...
void SLN_STREAMER_Resume(streamer_handle_t *handle)
{
    xSemaphoreTake(audioBufMutex, portMAX_DELAY);
    SLN_STREAMER_RestorePlayback();
    xSemaphoreGive(audioBufMutex);

    handle->audioPaused = false;

    SLN_STREAMER_Start(handle);
}
...
void SLN_STREAMER_Pause(streamer_handle_t *handle)
{
    ...
    vTaskDelay(100);
    handle->audioPlaying = false;
    handle->audioPaused  = true;
    streamer_set_state(handle->streamer, 0, STATE_NULL, true);

    xSemaphoreTake(audioBufMutex, portMAX_DELAY);
    SLN_STREAMER_BackupPlayback();
    xSemaphoreGive(audioBufMutex);
