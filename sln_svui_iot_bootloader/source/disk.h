...
/*!
 * @brief Enter MSD Mode.
 *
 */
void PerformMSD(void);

/*!
 * @brief Set fallback variable to true when in fallback mode.
 *
 * @param state Fallback enabled/disabled.
 */
void SLN_MSD_FallbackSet(bool state);

/*!
 * @brief Check if the bootloader is in fallback mode.
 *
 * @return Return true if the bootloader is in fallback mode, false otherwise.
 */
