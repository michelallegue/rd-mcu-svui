 ...
 ******************************************************************************/
#if ENABLE_ENCRYPTION
extern uint8_t s_tempBuffer[FLASH_SECTOR_SIZE];
...
#define ERASED_BLOCK_MAP_SIZE_BYTES (FICA_IMG_APP_A_SIZE / FLASH_BLOCK_SIZE / NUM_ERASED_BLOCKS_PER_SLOT)
#define ERASED_PADDING_BLOCK_CNT    2
static uint8_t s_usedBlocksMap[ERASED_BLOCK_MAP_SIZE_BYTES] = {0};

/*******************************************************************************
 * Code
 ******************************************************************************/

int32_t FICA_app_program_read(uint8_t *buffer, uint32_t offset, uint32_t size)
{
    status_t status = SLN_FLASH_NO_ERROR;

    if ((offset > s_newAppImgMaxSize) || (size > s_newAppImgMaxSize) || ((offset + size) > s_newAppImgMaxSize))
    {
        status = SLN_FLASH_ERROR;
    }

    SLN_Read_Flash_At_Address((s_newAppImgStartAddr + offset), buffer, size);
...
int32_t FICA_app_program_ext_init(int32_t newimgtype)
{
    ...
    memset(s_usedBlocksMap, 0, sizeof(s_usedBlocksMap));
...
int32_t FICA_app_program_ext_abs(uint32_t offset, uint8_t *bufptr, uint32_t writelen)
{
    ...
    {
        /* Mark block as used */
        uint32_t sizeVerified          = writelen;
        uint32_t offsetAddressVerified = pagestartaddr + pagenumrem;
        while (sizeVerified > 0)
        {
            blockSet = (offsetAddressVerified / FLASH_BLOCK_SIZE) / NUM_ERASED_BLOCKS_PER_SLOT;
            blockId  = (offsetAddressVerified / FLASH_BLOCK_SIZE) % NUM_ERASED_BLOCKS_PER_SLOT;
            s_usedBlocksMap[blockSet] |= (1 << blockId);
            if (sizeVerified > FLASH_BLOCK_SIZE)
            {
                sizeVerified -= FLASH_BLOCK_SIZE;
                offsetAddressVerified += FLASH_BLOCK_SIZE;
            }
            else
            {
                sizeVerified = 0;
    ...
    while (curextlen < writelen)
    {
        ...
        while (idx < EXT_FLASH_PROGRAM_PAGE)
        {
            ...
            {
                /* Page is already written. Erase the block.*/
                doErase = true;
                break;
            }
        }
...
int32_t FICA_app_program_ext_finalize()
{
    ...
    /* Application was written to the bank.
     * Erase extra ERASED_PADDING_BLOCK_CNT blocks. */
    uint32_t erasedPaddingBlocksCnt = 0;
    for (uint32_t blockSet = 0; blockSet < ERASED_BLOCK_MAP_SIZE_BYTES; blockSet++)
    {
        if (erasedPaddingBlocksCnt == ERASED_PADDING_BLOCK_CNT)
        {
            break;
        }

        for (uint32_t blockId = 0; blockId < NUM_ERASED_BLOCKS_PER_SLOT; blockId++)
        {
            if (erasedPaddingBlocksCnt == ERASED_PADDING_BLOCK_CNT)
            {
                break;
            ...
            if ((s_usedBlocksMap[blockSet] & (1 << blockId)) == 0)
            {
                FICA_Erase_Bank(s_newAppImgStartAddr + ((blockSet * NUM_ERASED_BLOCKS_PER_SLOT) + blockId) * FLASH_BLOCK_SIZE, FLASH_BLOCK_SIZE);
                s_usedBlocksMap[blockSet] |= (1 << blockId);

                erasedPaddingBlocksCnt++;
            }
        }
    }
...
static int32_t FICA_Verify_Certificate_From_Buffer(uint8_t *certPem)
{
    int32_t status     = SLN_FLASH_FS_OK;
    uint8_t *caRootPem = NULL;
    uint32_t caRootLen = 0;

    if (NULL == certPem)
    {
        status = SLN_FLASH_FS_EINVAL;
    }
...
int32_t FICA_Save_Certificate(int32_t imgType)
{
    ...
    if (SLN_FLASH_NO_ERROR == status)
    {
        ...
        {
            status = sln_flash_fs_ops_save(CRED_SIGNING_CERT, s_newAppCertificate,
                                         safe_strlen((char *)s_newAppCertificate, MAX_CERT_LEN) + 1);
        }
        else if (FICA_IMG_TYPE_APP_A == imgType)
        {
            status = sln_flash_fs_ops_save(APP_A_SIGNING_CERT, s_newAppCertificate,
                                         safe_strlen((char *)s_newAppCertificate, MAX_CERT_LEN) + 1);
        }
        else if (FICA_IMG_TYPE_APP_B == imgType)
        {
            status = sln_flash_fs_ops_save(APP_B_SIGNING_CERT, s_newAppCertificate,
                                         safe_strlen((char *)s_newAppCertificate, MAX_CERT_LEN) + 1);
        }
        else
        {
...
int32_t FICA_Invalidate_Certificate(int32_t imgType)
{
    ...
    if (SLN_FLASH_NO_ERROR == status)
    {
        if (FICA_IMG_TYPE_BOOTLOADER == imgType)
        {
            status = sln_flash_fs_ops_save(CRED_SIGNING_CERT, dummy_cert, sizeof(dummy_cert));
        }
        else if (FICA_IMG_TYPE_APP_A == imgType)
        {
            status = sln_flash_fs_ops_save(APP_A_SIGNING_CERT, dummy_cert, sizeof(dummy_cert));
        }
        else if (FICA_IMG_TYPE_APP_B == imgType)
        {
            status = sln_flash_fs_ops_save(APP_B_SIGNING_CERT, dummy_cert, sizeof(dummy_cert));
        }
        else
        {
            status = SLN_FLASH_ERROR;
...
int32_t FICA_Erase_Certificate(int32_t imgType)
{
    ...
    if (SLN_FLASH_NO_ERROR == status)
    {
        if (FICA_IMG_TYPE_BOOTLOADER == imgType)
        {
            status = sln_flash_fs_ops_erase(CRED_SIGNING_CERT);
        }
        else if (FICA_IMG_TYPE_APP_A == imgType)
        {
            status = sln_flash_fs_ops_erase(APP_A_SIGNING_CERT);
        }
        else if (FICA_IMG_TYPE_APP_B == imgType)
        {
            status = sln_flash_fs_ops_erase(APP_B_SIGNING_CERT);
        }
        else
        {
            status = SLN_FLASH_ERROR;
...
int32_t FICA_Verify_Signature(int32_t imgType)
{
    ...
    if ((SLN_FLASH_FS_OK == status) || (SLN_FLASH_FS_EENCRYPT2 == status))
    {
...
int32_t FICA_Verify_OTA_Image_Entry_Point(int32_t imgType)
{
    ...
    if (SLN_FLASH_NO_ERROR == status)
    {
        ...
        if ((0xFFFFFFFF == appEntryPointAddr) || /* invalid address */
            ...
            (appEntryPointAddr <= SLN_Flash_Get_Read_Address(FICA_IMG_APP_A_ADDR)) || /* must be greater than Bank address, otherwise we could have BankA app in BankB */
            ((appEntryPointAddr - SLN_Flash_Get_Read_Address(FICA_IMG_APP_A_ADDR)) > 0x10000)) /* bigger difference should be a clear indicator of BankB app in BankA */
#else
        if ((0xFFFFFFFF == appEntryPointAddr) || /* invalid address */
            (0x00000000 == appEntryPointAddr) || /* invalid address */
            (appEntryPointAddr <= appVectorTable) || /* must be greater than Bank address, otherwise we could have BankA app in BankB */
            ((appEntryPointAddr - appVectorTable) > 0x10000)) /* bigger difference should be a clear indicator of BankB app in BankA */
#endif /* ENABLE_REMAP */
        {
            status = SLN_FLASH_ERROR;
        }
