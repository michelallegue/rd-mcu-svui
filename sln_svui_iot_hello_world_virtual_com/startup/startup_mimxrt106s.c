//*****************************************************************************
// MIMXRT106S startup code for use with MCUXpresso IDE
//
// Version : 310320
//*****************************************************************************
//
...
#if defined(CONFIG_FLEXRAM_AT_STARTUP) && (CONFIG_FLEXRAM_AT_STARTUP == 1)
//*****************************************************************************
// FlexRAM mapping symbols and method
//*****************************************************************************
...
static void map_flexram(void)
{
    ...
    unsigned int ocSize_non_cacheable = (unsigned int)__top_SRAM_OC_NON_CACHEABLE - (unsigned int)__base_SRAM_OC_NON_CACHEABLE;
    ...
    unsigned int dtcBankCount = dtcSize / 0x8000U;
    unsigned int itcBankCount = itcSize / 0x8000U;
    unsigned int ocBankCount = ocSize / 0x8000U - 16;

    flexram_allocate_ram_t cfg = {ocBankCount, dtcBankCount, itcBankCount};

    FLEXRAM_AllocateRam(&cfg);
}

#define SAFE_STACK_TOP (0x20200000 + 0x8000) // Temporary stack pointer to end of first block of SRAM OC (always there)

#else

#define SAFE_STACK_TOP _vStackTop // Safe stack pointer entry if not reconfiguring

#endif // CONFIG_FLEXRAM_AT_STARTUP
//*****************************************************************************
#if defined (__cplusplus)
} // extern "C"
#endif
...
void (* const g_pfnVectors[])(void) = {
    // Core Level - CM7
    (vector_entry_t)SAFE_STACK_TOP,    // The initial stack pointer
...
void ResetISR(void) {
    ...
    // Determine the space used on the temp stack
    volatile unsigned int vOldStackTop = (unsigned int)SAFE_STACK_TOP;
    volatile unsigned int vCurrStack = (CONTROL_SPSEL_Msk & __get_CONTROL( )) ? __get_PSP() : __get_MSP();
    volatile unsigned int vOldStackUsed = vOldStackTop - vCurrStack;
    ...
    (CONTROL_SPSEL_Msk & __get_CONTROL( )) ? __set_PSP(vNewStackTop) : __set_MSP(vNewStackTop);
    __ISB();
