...
#define PARAM_SECTOR FLASH_SECTOR_SIZE // 0x1000U // Set to 4 KB paramter sector size if avilable

static mbedtls_x509_crt __attribute__((section(".bss.$SRAM_OC_CACHEABLE")))s_caChain;
static mbedtls_x509_crt __attribute__((section(".bss.$SRAM_OC_CACHEABLE")))s_verifCert;

__attribute__((section(".bss.$SRAM_OC_CACHEABLE"))) __attribute__((aligned(4)))
    ...
    uint8_t *msg, size_t msglen, uint8_t *hash, uint32_t hashSize, mbedtls_md_type_t mdType)
{
    ...
    else if (mdCtx.md_info->size > hashSize)
    {
        ret = SLN_AUTH_NO_MEM;
    }
...
int32_t SLN_AUTH_Generate_Hash(uint8_t *msg, size_t msglen, uint8_t *hash, uint32_t hashSize, mbedtls_md_type_t mdType)
{
    ...
    if (SLN_AUTH_OK == ret)
    {
        ...
        if (mdCtx.md_info->size > hashSize)
        {
            ret = SLN_AUTH_NO_MEM;
        }

        flash_page = (uint8_t *)pvPortMalloc(FLASH_PAGE_SIZE);
        ...
        if (SLN_AUTH_OK == ret)
        {
            while (to_read)
            {
                memset(flash_page, 0, FLASH_PAGE_SIZE);
                current_chunk = to_read > FLASH_PAGE_SIZE ? FLASH_PAGE_SIZE : to_read;
...
int32_t SLN_AUTH_Verify_Signature(uint8_t *vfPem, uint8_t *msg, size_t msglen, uint8_t *msgsig)
{
    ...
    if (SLN_AUTH_OK == ret)
    {
        ...
        hash = (char *)pvPortMalloc(mdInfo->size);
...
#if SLN_AUTH_SLOW_HASH
        ret = SLN_AUTH_Generate_Hash(msg, msglen, (uint8_t *)hash, mdInfo->size, mdType);
#else
        ret = SLN_AUTH_Generate_HashFast(msg + FLEXSPI_AMBA_BASE, msglen, (uint8_t *)hash, mdInfo->size, mdType);
#endif /* SLN_AUTH_SLOW_HASH */

        if (SLN_AUTH_OK == ret)
        {
            status = mbedtls_pk_verify(&(s_verifCert.pk), mdType, (uint8_t *)hash, mdInfo->size, msgsig, RSA_SIG_LEN);
            if (status)
            {
...
int32_t SLN_AUTH_Verify_Hash(uint8_t *msg, size_t msglen, uint8_t *pre_write_hash)
{
    ...
    if (SLN_AUTH_OK == ret)
    {
        ...
        if (NULL != hash)
        {
            memset(hash, 0, mdCtx.md_info->size);
            vPortFree(hash);
            hash = NULL;
        }
