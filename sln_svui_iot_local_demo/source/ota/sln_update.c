...
static uint8_t __attribute__((section(".bss.$SRAM_OC_NON_CACHEABLE")))s_newAppCertificate[MAX_CERT_LEN]   = {0};
...
status_t SLN_Update_VerifyImgBank(fica_img_type_t imgType, uint8_t *imgSig)
{
    ...
    if ((imgSig == NULL) || (imgType != s_preparedImgType))
    {
        status = kStatus_InvalidArgument;
    }
    ...
    if (kStatus_Success == status)
    {
        ...
        offsetAdjust = imgSize - MAX_CERT_LEN;

        offset = imgStartingAddr + offsetAdjust;

        SLN_Read_Flash_At_Address(offset, s_newAppCertificate, MAX_CERT_LEN);

        // Verify this certificate
        fica_error |= FICA_Verify_Certificate_From_Buffer(s_newAppCertificate);

        if (SLN_FLASH_NO_ERROR == fica_error)
        {
            fica_error = SLN_AUTH_Verify_Signature(s_newAppCertificate, (uint8_t *)imgStartingAddr, imgSize - MAX_CERT_LEN, (uint8_t *)imgSig);
        }
        if (SLN_FLASH_NO_ERROR == fica_error)
        {
            fica_error = FICA_Save_Certificate(imgType, s_newAppCertificate);
        }
        if (SLN_FLASH_NO_ERROR == fica_error)
        {
            fica_error = FICA_Save_Signature((uint8_t *)imgSig);
        }
    }
