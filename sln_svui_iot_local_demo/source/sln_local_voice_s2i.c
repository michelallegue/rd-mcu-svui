...
/* Used models */
#include "VIT_Model_en_Hvac.h"
#include "VIT_Model_en_Oven.h"
...
static VIT_ReturnStatus_en VIT_Init(void)
{
    ...
    switch (appAsrShellCommands.demo)
    {
        case ASR_CMD_CHANGE_DEMO:
            VIT_Status = VIT_SetModel((const PL_UINT8 *)VIT_Model_en_hvac, VIT_MODEL_IN_FAST_MEM);
            break;
        case ASR_S2I_HVAC:
            VIT_Status = VIT_SetModel((const PL_UINT8 *)VIT_Model_en_hvac, VIT_MODEL_IN_FAST_MEM);
            break;
        case ASR_S2I_OVEN:
            VIT_Status = VIT_SetModel((const PL_UINT8 *)VIT_Model_en_oven, VIT_MODEL_IN_FAST_MEM);
            break;
        case ASR_S2I_HOME:
...
void local_voice_task(void *arg)
{
    ...
    if ((appAsrShellCommands.status != WRITE_SUCCESS) ||
        ((appAsrShellCommands.demo != ASR_S2I_HVAC) && (appAsrShellCommands.demo != ASR_S2I_OVEN) && (appAsrShellCommands.demo != ASR_S2I_HOME)) ||
