...
#endif
    rv = wlan_set_chanlist_and_txpwrlimit(&chanlist_2g_cfg, (wifi_txpwrlimit_t *)(&tx_pwrlimit_2g_cfg));
    if (rv != WM_SUCCESS)
    {
...
#ifdef CONFIG_5GHz_SUPPORT
    rv = wlan_set_chanlist_and_txpwrlimit(&chanlist_5g_cfg, (wifi_txpwrlimit_t *)&tx_pwrlimit_5g_cfg);
    if (rv != WM_SUCCESS)
    {
